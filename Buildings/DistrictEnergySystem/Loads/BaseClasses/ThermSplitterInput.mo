...
model ThermSplitterInput
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput splitFactorVec[nOut](each unit="1")
  "Vector of split factors (between 0 and 1)" annotation (Placement(
        transformation(
        extent={{-20,-20},{20,20}},
        rotation=0,
        ...
        origin={0,120})));
protected
  Real splitFactor[nOut, nIn];
equation
  splitFactor = {{splitFactorVec[i]} for i in 1:nOut};
