...
model CouplingTimeSeriesSplit2Loads "Example illustrating the coupling of a time series building model to a fluid loop"
  import Buildings;
  ...
  Buildings.DistrictEnergySystem.Loads.BaseClasses.HeatingOrCoolingSplit couHea(
    ...
    T_a_nominal=318.15,
    T_b_nominal=313.15,
    TLoa_nominal={20 + 273.15,25 + 273.15})  annotation (Placement(transformation(extent={{-20,40},{0,20}})));
...
equation
  ...
  connect(gai2.y, conPIDHea.u_s) annotation (Line(points={{-139,38},{-128,38}}, color={0,0,127}));
  ...
  connect(gai2.u, mulSum.y) annotation (Line(points={{-162,38},{-179,38}}, color={0,0,127}));
  ...
  connect(bui.Q_flowHeaReq, mulSum.u[1:2])
    annotation (Line(points={{61.1,-27},{-219.45,-27},{-219.45,37},{-202,37}}, color={0,0,127}));
  connect(bui.Q_flowHeaAct, mulSum1.u[1:2])
    annotation (Line(points={{61.1,-21},{1.55,-21},{1.55,-7},{-58,-7}}, color={0,0,127}));
  connect(bui.heaPorHea, couHea.heaPorLoa) annotation (Line(points={{38,-23},{-10,-23},{-10,20}}, color={191,0,0}));
  connect(bui.Q_flowHeaReq, couHea.Q_flow)
    annotation (Line(points={{61.1,-27},{-28.45,-27},{-28.45,26},{-22,26}}, color={0,0,127}));
  annotation (Diagram(
    ...
    __Dymola_Commands(file=
        ...
        "Simulate and plot"));
end CouplingTimeSeriesSplit2Loads;
